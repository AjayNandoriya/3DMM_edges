function [im,test,depthMap] = render_face_normals(FV, im,R,t,s)
...
xbuffer     = -inf(height, width);
ybuffer     = -inf(height, width);
...
for i = 1: Nfaces
    ...
    if minx(i) <= maxx(i) && miny(i) <= maxy(i)
        ...
        if any(test)
            ...
            pzx = Zx(v1(i)) * w1 + Zx(v2(i)) * w2 + Zx(v3(i)) * w3;
            pzy = Zy(v1(i)) * w1 + Zy(v2(i)) * w2 + Zy(v3(i)) * w3;
...
%                if pz(j) <= -near && pz(j) >= -far
                    ...
                    if pz(j) > zbuffer(py(j), px(j))
                        ...
                        xbuffer(py(j), px(j))   = pzx(j);
                        ybuffer(py(j), px(j))   = pzy(j);
                        fbuffer(py(j), px(j))   = i;
...
clear UV Z px py pz minx maxx miny maxy
...
ybuffer(~test)=NaN;
zbuffer(~test)=NaN;
xbuffer = xbuffer - min(xbuffer(:));
ybuffer = ybuffer - min(ybuffer(:));
zbuffer = zbuffer - min(zbuffer(:));

xbuffer = xbuffer./max(xbuffer(:));
ybuffer = ybuffer./max(ybuffer(:));
zbuffer = zbuffer./max(zbuffer(:));
depthMap = xbuffer;
depthMap(:,:,2) = ybuffer;
depthMap(:,:,3) = zbuffer;

clear xbuffer ybuffer zbuffer
% Compute the edge vectors
e1s = V2(v2(f), :) - V2(v1(f), :);
e2s = V2(v3(f), :) - V2(v1(f), :);
e3s = V2(v2(f), :) - V2(v3(f), :);
