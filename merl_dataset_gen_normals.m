...
testdir='C:\Users\qcri\Documents\Ajay\Face\dataset\mean_img_rotate\';
testdir='C:\Users\qcri\Documents\Ajay\Face\SOA\sig_2017_mtp\img\';
for img_id = 5
    fname = strcat(testdir,sprintf('mean_%03d_1_15.jpg',img_id));
    if(~exist(fname,'file'))
        fprintf('%s not found.\n',fname);
        continue;
    end
...
normals_fname = fullfile(testdir,sprintf('normals_%03d.png',img_id));
masks_fname = fullfile(testdir,sprintf('masks_%03d.png',img_id));
d_fname = fullfile(testdir,sprintf('depth_%03d.png',img_id));
imwrite((normals+1)/2,normals_fname);
imwrite(double(mask),masks_fname);
imwrite(depthMap,d_fname);
end
